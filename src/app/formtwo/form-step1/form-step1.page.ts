...
export class FormStep1Page implements OnInit {
  ...
  async form(event){
    ...
    this.dataStorage[this.numberId].S1A = value;
   
    await this.storage.set('formshop',this.dataStorage);
    await this.router.navigateByUrl('formtwo/form-step2/'+this.numberId);
  }
  async backPage(){
    this.router.navigateByUrl('formtwo/form4/'+this.numberId);
  }
}
