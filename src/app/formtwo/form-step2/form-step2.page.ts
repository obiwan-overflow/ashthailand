...
export class FormStep2Page implements OnInit {
  ...
  async form(event){
    ...
    this.dataStorage[this.numberId].S2A = value;
    
    await this.storage.set('formshop',this.dataStorage);
    await this.router.navigateByUrl('formtwo/form-step3/'+this.numberId);
  }
  async backPage(){
    this.router.navigateByUrl('formtwo/form-step1/'+this.numberId);
  }
}
